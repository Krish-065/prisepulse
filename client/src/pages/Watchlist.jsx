...
export default function Watchlist() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const { data } = await axios.get(BASE + '/watchlist', { headers: { Authorization: 'Bearer ' + token } });
        setWatchlist(data);
  ...
  useEffect(() => {
    ...
    const matched = NSE_STOCKS.filter(s => s.sym.toUpperCase().startsWith(q) || s.name.toUpperCase().includes(q)).slice(0, 10);
    if (q.length >= 2 && !matched.find(s => s.sym === q)) matched.push({ sym: q, name: 'Search "' + q + '" on NSE' });
  ...
  const addStock = async (sym) => {
    ...
    if (!symbol) return;
    if (watchlist.symbols.includes(symbol)) { setAddError(symbol + ' already in watchlist'); setTimeout(() => setAddError(''), 3000); setSearch(''); setShowDrop(false); return; }
    setAdding(symbol); setAddError(''); setSearch(''); setShowDrop(false);
    try {
      const { data } = await axios.post(BASE + '/watchlist/add', { symbol }, { headers: { Authorization: 'Bearer ' + token } });
      setWatchlist(data);
      fetchStockPrices(data.symbols);
    } catch (err) { setAddError('Could not add ' + symbol + '. Check if it is a valid NSE symbol.'); setTimeout(() => setAddError(''), 4000); }
    setAdding('');
  };

  const removeStock = async (symbol) => {
    try {
      const { data } = await axios.post(BASE + '/watchlist/remove', { symbol }, { headers: { Authorization: 'Bearer ' + token } });
      setWatchlist(data);
  ...
  const addCrypto = async (id) => {
    ...
    try {
      const { data } = await axios.post(BASE + '/watchlist/crypto/add', { id }, { headers: { Authorization: 'Bearer ' + token } });
      setWatchlist(data);
      fetchCryptoPrices(data.cryptos);
    } catch (err) { console.log('Crypto add error:', err); }
  };

  const removeCrypto = async (id) => {
    try {
      const { data } = await axios.post(BASE + '/watchlist/crypto/remove', { id }, { headers: { Authorization: 'Bearer ' + token } });
      setWatchlist(data);
  ...
  const addCommodity = async (id) => {
    ...
    try {
      const { data } = await axios.post(BASE + '/watchlist/commodity/add', { id }, { headers: { Authorization: 'Bearer ' + token } });
      setWatchlist(data);
      fetchCommPrices();
  ...
  const removeCommodity = async (id) => {
    try {
      const { data } = await axios.post(BASE + '/watchlist/commodity/remove', { id }, { headers: { Authorization: 'Bearer ' + token } });
      setWatchlist(data);
            ...
            <div className="relative">
              ...
              {showDrop && suggestions.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-lg overflow-hidden z-50 shadow-xl">
                  {suggestions.map((s, i) => (
                    <div key={i} onClick={() => !watchlist.symbols.includes(s.sym) && addStock(s.sym)}
                      className={'flex items-center justify-between px-4 py-3 border-b border-gray-700 last:border-0 transition-colors ' +
                        (watchlist.symbols.includes(s.sym) ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer hover:bg-gray-700')}>
                      ...
                      <span className="text-green-400 text-xs font-mono flex-shrink-0 ml-2">
                        {watchlist.symbols.includes(s.sym) ? 'Added' : adding === s.sym ? 'Adding...' : '+ Add'}
                      </span>
                    </div>
                  ))}
      ...
      {tab === 'crypto' && (
        <div>
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-5">
            ...
            />
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-52 overflow-y-auto pr-1">
              {filteredCrypto.map(c => {
                ...
                return (
                  <div key={c.id} onClick={() => !inList && addCrypto(c.id)}
                    ...
                    <span className={'text-xs font-mono ' + (inList ? 'text-green-400' : 'text-gray-500')}>
                      {inList ? '✓' : '+'}
                    </span>
                  </div>
                );
