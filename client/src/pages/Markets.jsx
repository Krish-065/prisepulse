...
export default function Markets() {
  ...
  var [liveIdx, setLiveIdx]         = useState({
    NIFTY: 0, SENSEX: 0, BANK_NIFTY: 0, NIFTY_IT: 0,
    NIFTY_CHANGE: 0, SENSEX_CHANGE: 0, BANK_NIFTY_CHANGE: 0, NIFTY_IT_CHANGE: 0,
  });

  useEffect(function() {
    socket.on('price-update', function(d) { setLiveIdx(d); });
    ...
    var load = async function() {
      ...
      var results = await Promise.allSettled([
        ...
        axios.get('https://newsapi.org/v2/everything?q=india+stock+market+nifty+sensex+economy+RBI+crypto&sortBy=publishedAt&pageSize=15&language=en&apiKey=2254829406fb447d91aa57aaadce71ae'),
      ...
      if (results[2].status === 'fulfilled') {
        var raw = results[2].value.data;
        // Handle both backend format and direct NewsAPI format
        if (raw && raw.articles) {
          setNews(raw.articles
            .filter(function(a) { return a.title && a.title !== '[Removed]'; })
            .map(function(a) { return { title: a.title, source: a.source.name, url: a.url, time: a.publishedAt }; })
          );
        } else {
          setNews(raw);
        }
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        <div className="text-white text-xl font-bold font-mono">
          {Number(props.value) > 0
            ? Number(props.value).toLocaleString('en-IN', { maximumFractionDigits: 2 })
            : 'Loading...'}
        </div>
        <div className={'text-xs font-mono mt-1 ' + (Number(props.change) >= 0 ? 'text-green-400' : 'text-red-400')}>
          {Number(props.change) >= 0 ? '+' : ''}{Number(props.change).toFixed(2)}%
        </div>
      </div>
    );
  };
    ...
    <div className="p-4 max-w-7xl mx-auto">
      ...
      {status && (
        <div className={
          ...
          (status.isOpen ? 'bg-green-400/10 text-green-400 border border-green-400/20' : 'bg-gray-800 text-gray-400 border border-gray-700')
        }>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <IndexCard label="NIFTY 50"   value={liveIdx.NIFTY}      change={liveIdx.NIFTY_CHANGE}      />
        <IndexCard label="SENSEX"     value={liveIdx.SENSEX}     change={liveIdx.SENSEX_CHANGE}     />
        <IndexCard label="BANK NIFTY" value={liveIdx.BANK_NIFTY} change={liveIdx.BANK_NIFTY_CHANGE} />
        <IndexCard label="NIFTY IT"   value={liveIdx.NIFTY_IT}   change={liveIdx.NIFTY_IT_CHANGE}   />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-800 flex justify-between items-center">
              ...
              <button onClick={function() { setActiveTab('gainers'); }} className="text-green-400 text-xs">See all</button>
            </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-800 flex justify-between items-center">
              ...
              <button onClick={function() { setActiveTab('losers'); }} className="text-green-400 text-xs">See all</button>
            </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          {news.length === 0
            ...
            : news.map(function(n, i) {
                return (
                  <div key={i} className="px-5 py-4 border-b border-gray-800 last:border-0 hover:bg-gray-800/30 cursor-pointer">
      ...
      {activeTab === 'commodities' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {commodities
            ...
            : <div className="col-span-3 text-gray-500 text-xs text-center font-mono py-8">Loading...</div>
          }
        </div>
      )}
